...
export class InspecteurComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.inspecteurService
      ...
      .subscribe({
        next: (res: HttpResponse<IInspecteur[]>) => {
  ...
  trackId(index: number, item: IInspecteur): number {
    return item.id!;
  }
  ...
  delete(inspecteur: IInspecteur): void {
    const modalRef = this.modalService.open(InspecteurDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.inspecteur = inspecteur;
  ...
  protected onSuccess(data: IInspecteur[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
