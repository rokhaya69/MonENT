...
/**
 * REST controller for managing {@link sn.thiane.mefpai.ent.domain.Departement}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class DepartementResource {
    ...
    public ResponseEntity<Departement> partialUpdateDepartement(
        ...
        Optional<Departement> result = departementRepository
            ...
            .map(existingDepartement -> {
                if (departement.getNomDept() != null) {
                    existingDepartement.setNomDept(departement.getNomDept());
                }
                if (departement.getAutreDept() != null) {
                    existingDepartement.setAutreDept(departement.getAutreDept());
                }
    ...
    public ResponseEntity<List<Departement>> getAllDepartements(
        ...
        if (eagerload) {
            page = departementRepository.findAllWithEagerRelationships(pageable);
        } else {
            page = departementRepository.findAll(pageable);
        }
