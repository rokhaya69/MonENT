...
import sn.thiane.mefpai.ent.repository.EtablissementRepository;
...
/**
 * REST controller for managing {@link sn.thiane.mefpai.ent.domain.Etablissement}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class EtablissementResource {
    ...
    public ResponseEntity<List<Etablissement>> getAllEtablissements(
        ...
        if (eagerload) {
            page = etablissementRepository.findAllWithEagerRelationships(pageable);
        } else {
            page = etablissementRepository.findAll(pageable);
        }
