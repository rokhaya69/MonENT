...
/**
 * REST controller for managing {@link sn.thiane.mefpai.ent.domain.Commune}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class CommuneResource {
    ...
    public ResponseEntity<List<Commune>> getAllCommunes(
        ...
        Page<Commune> page;
        if (eagerload) {
            page = communeRepository.findAllWithEagerRelationships(pageable);
        } else {
    ...
    public ResponseEntity<Commune> getCommune(@PathVariable Long id) {
        ...
        Optional<Commune> commune = communeRepository.findOneWithEagerRelationships(id);
